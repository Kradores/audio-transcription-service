...
def test_nvidia_configuration_is_valid() -> None:
    repository_root = Path(__file__).resolve().parents[4]

    config_path = repository_root / "config" / "config.nvidia.example.yaml"
    ...
    assert settings.whisper.runtime is WhisperRuntime.NVIDIA
