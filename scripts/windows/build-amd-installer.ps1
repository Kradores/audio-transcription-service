...
function Find-InnoSetupCompiler {
    if ($env:INNO_SETUP_COMPILER) {
        if (-not (
            Test-Path `
                -LiteralPath $env:INNO_SETUP_COMPILER
        )) {
            throw (
...
if ($LASTEXITCODE -ne 0) {
    throw (
        "AMD application build failed with exit code " +
        $LASTEXITCODE
    )
}
...
if (-not (
    ...
    throw (
        "Expected packaged AMD executable was not found: " +
...
Write-Host ""
Write-Host "=== Compile AMD installer ===" `
    -ForegroundColor Cyan
...
Write-Host ""
Write-Host "[OK] AMD Windows installer built" `
